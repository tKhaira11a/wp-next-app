...
export default function Edit( { attributes, setAttributes, clientId  } ) {
	...
	const createCptEntry = async () => {
		...
		try {
			...
			const cptName = "timeline_container";
			const postCategory = "postType";

			const newPostReccord = {
				title: `$-timeline-container-${Uuid}`,
				status: 'publish',
				meta: {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'timeline-container-block' ) }>
					<TextareaControl
						label={ __(
			...
			<div {...blockProps}>
				<InnerBlocks
					allowedBlocks={['nextpress-block/timeline-item']}
					template={[['nextpress-block/timeline-item']]}
					templateLock={false}
					className={"wp-block-timeline-container"}
				/>
			</div>
		</>
	);
