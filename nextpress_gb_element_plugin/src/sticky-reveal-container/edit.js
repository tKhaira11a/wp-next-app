...
export default function Edit( { attributes, setAttributes, clientId  } ) {
	const { cptId, instanceId, itemIds, style = {css : ""}  } = attributes;
	...
	const createCptEntry = async () => {
		...
		try {
			...
			const cptName = "sticky_rev_container";
			const postCategory = "postType";

			const newPostReccord = {
				title: `$-sticky-reveal-container-${Uuid}`,
				status: 'publish',
				meta: {
					attributes: JSON.stringify(attributes),
					item_ids: JSON.stringify(childLabels)
				}
			};
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={ __( 'Settings', 'sticky-reveal-container-block' ) }>

					<TextareaControl
						label={ __(
							'Additional CSS for Element. No selectors!',
							'sticky-reveal-container-block'
						) }
						help="CSS-Styles"
						value={ style.css }
			...
			<div {...blockProps}>
				<ul>
					<InnerBlocks
						allowedBlocks={['nextpress-block/sticky-reveal-item']}
						template={[['nextpress-block/sticky-reveal-item']]}
						templateLock={false}
						className={"wp-block-sticky-reveal-container"}
					/>
				</ul>
			</div>
		</>
