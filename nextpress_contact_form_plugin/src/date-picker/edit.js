...
import { DatePickerInput } from "./date-picker.js";
...
export default function Edit({ attributes, setAttributes, clientId }) {
	...
	const [dateTimeProxy, setDateTimeProxy] = useState(new Date());
	...
	const createCptEntry = async () => {
		...
		try {
			const cptName = "date_picker";
			const postCategory = "postType";

			const newPostReccord = {
				title: `$-date-picker-${Uuid}`,
				status: 'publish',
				meta: {
					label: label,
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Settings', 'date-picker-block')}>
					<TextControl
						label={__('Field-Name (no spaces)', 'date-picker-block')}
						value={fieldName}
						onChange={(newValue) => {
			...
			<div {...blockProps} >
				...
				<DatePickerInput
					label={label}
					currentDate={dateTimeProxy}
					onChange={(value) => setDateTimeProxy(new Date(value))}
				/>
			</div>
		</>
	);
