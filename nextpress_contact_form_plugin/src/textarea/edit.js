...
export default function Edit({ attributes, setAttributes, clientId  }) {
	...
	const [txAreaValue, setTxAreaValue] = useState("");
...
const updateCptEntry = async () => {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Settings', 'textarea-block')}>
					<TextControl
						label={__('Field-Name (no spaces)', 'textarea-block')}
						value={fieldName}
						onChange={(newValue) => {
			...
			<div {...blockProps} >
				...
				<TextareaControl
					value={txAreaValue}
					onChange={(value) => handleAttributeChange('txAreaValue', value)}
				/>
			</div>
		</>
	);
