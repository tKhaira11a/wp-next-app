...
import {CheckboxControl, PanelBody, TextareaControl, TextControl} from "@wordpress/components";
...
const updateCptEntry = async () => {
	...
	return (
		<>
			<InspectorControls>
				<PanelBody title={__('Settings', 'checkbox-block')}>
					<TextControl
						label={__('Field-Name (no spaces)', 'checkbox-block')}
						value={fieldName}
						onChange={(newValue) => {
			...
			<div {...blockProps} >
				<div style={{ display: 'flex', gap: '10px', alignItems: 'flex-end', maxWidth: '200px' }}  >
					<div style={{ flex: 1 }}>
						<RichText
							tagName="label"
							value={label}
